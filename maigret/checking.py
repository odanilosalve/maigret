...
class SimpleAiohttpChecker(CheckerBase):
    ...
    async def _make_request(
        ...
        try:
            _http_methods = {'get': 'get', 'post': 'post', 'head': 'head'}
            request_method = getattr(session, _http_methods.get(method.lower(), 'get'))
...
def process_site_result(
    ...
    if not keywords or not html_text:
        ...
        keyword_match_status = KeywordMatchStatus.KEYWORD_FOUND if keywords_found else KeywordMatchStatus.KEYWORDS_NOT_FOUND
        if keywords_found:
    ...
    if check_error:
        ...
        def _check_message():
            ...
            return MaigretCheckStatus.CLAIMED if (not is_absence_detected and is_presense_detected) else MaigretCheckStatus.AVAILABLE

        def _check_status_code():
            return MaigretCheckStatus.CLAIMED if 200 <= status_code < 300 else MaigretCheckStatus.AVAILABLE

        def _check_response_url():
            return MaigretCheckStatus.CLAIMED if (200 <= status_code < 300 and is_presense_detected) else MaigretCheckStatus.AVAILABLE
...
def make_site_result(
    ...
    if site.request_payload:
        payload = {k: v.format(username=username) if isinstance(v, str) else v for k, v in site.request_payload.items()}
